...
function buildLinkedInJobsSearchUrl(linkedInUrl, resumeKeywords) {
    const fallback = `https://www.linkedin.com/jobs/search?keywords=${encodeURIComponent(resumeKeywords)}&location=${encodeURIComponent('India')}`;
    if (!linkedInUrl) {
        return fallback;
    }
    try {
...
function buildNaukriSearchUrl(resumeKeywords) {
    const slug = String(resumeKeywords || 'software engineer')
        .toLowerCase()
        .replace(/[^a-z0-9\s-]/g, ' ')
        .trim()
        .replace(/\s+/g, '-');
    return `https://www.naukri.com/${slug || 'software-engineer'}-jobs?k=${encodeURIComponent(resumeKeywords || 'software engineer')}`;
}
...
exports.suggestJobs = async (req, res) => {
    try {
        ...
        const naukriUrl = buildNaukriSearchUrl(resumeKeywords);
